...
def _move_stock_to_delivered(db: Session, stock: models.Stock, actor: models.User) -> models.Delivered:
    existing = db.query(models.Delivered).filter(models.Delivered.Frame == stock.Frame).first()
    if existing:
        # Update the existing Delivered record instead of silently keeping stale data
        existing.EngineNoMotorNo = stock.EngineNoMotorNo
        existing.ModelVariant = stock.ModelVariant
        existing.ProductName = stock.ProductName
        existing.Color = stock.Color
        existing.ModelName = stock.ModelName
        existing.ManufacturingDate = stock.ManufacturingDate
        existing.Location = stock.Location
        existing.DeliveredDateTime = datetime.now(IST).replace(tzinfo=None)
        db.delete(stock)
        return existing
...
def get_current_user(username: Annotated[str, Depends(get_current_username)], db: Session = Depends(get_db)):
    # Super admin is not a DB row - build a transient (never added/committed) User object
    # so downstream code (is_admin, audit logs, etc.) works exactly like it does for real users.
    if SUPER_ADMIN_USERNAME and username == SUPER_ADMIN_USERNAME.lower():
        return models.User(
            id=0,
            first_name="Super",
            last_name="Admin",
...
def login_with_username_password(
    ...
    # Normalize whatever case the user typed - usernames are matched case-insensitively.
    username_input = (form_data.username or "").strip().lower()

    # --- Super admin path: not a DB row, checked against the hardcoded credentials above ---
    if SUPER_ADMIN_USERNAME and username_input == SUPER_ADMIN_USERNAME.lower():
        if form_data.password != SUPER_ADMIN_PASSWORD:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid credentials",
        ...
        access_token = create_access_token(
            data={"sub": SUPER_ADMIN_USERNAME.lower(), "role": "admin"},
            expires_delta=timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES),
        )
        refresh_token = create_access_token(
            data={"sub": SUPER_ADMIN_USERNAME.lower(), "role": "admin", "type": "refresh"},
            expires_delta=timedelta(minutes=REFRESH_TOKEN_EXPIRE_MINUTES),
        )
        return {
            "access_token": access_token,
    ...
    # Find user (case-insensitive match on stored username)
    user = db.query(models.User).filter(
        func.lower(models.User.username) == username_input
    ).first()
...
def admin_update_stock(
    ...
    if loc_norm == "delivered":
        ...
        # Write the final "Delivered" location-history row BEFORE the stock row
        # is moved/deleted, otherwise the frame's track history never shows
