...
#     DeliveredDateTime = Column('Delivered DateTime', DateTime, nullable=False, default=lambda: datetime.now(IST).replace(tzinfo=None))
...
class LocationLog(Base):
    ...
    mobile = Column(String, nullable=True)
    role = Column(String, nullable=True)
    # IST timestamp of when this log row was created (stored naive, already shifted to IST)
    logged_at = Column(DateTime, nullable=False, default=lambda: datetime.now(IST).replace(tzinfo=None))
